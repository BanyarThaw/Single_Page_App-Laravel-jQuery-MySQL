...
@section('sub_content')
        ...
        @foreach($guests as $guest)
            <div class="reception_user_list_mobile_detail">
              <h5 class="date_format_mobile">{{ $guest->created_at->format('d.n.Y') }}</h5>
                <a href="/guests/{{$guest->id}}" class="detail_anchor" id="myBtn">
                    <p class="name_mobile"> @php $words_limit = $words_count::words_limit($guest->name); echo $words_limit; @endphp</b>
                </a>
                <br>
            </div>
          <br>
    ...
    <div class="reception_search_form">
        <form action="/reception/check_out/search" method="post" autocomplete="off">
            {{ csrf_field() }}
        <input type="text" name="search" placeholder="Search Name" class="ajax_check_out_list" id="Search" autocomplete="false">
        </form>
